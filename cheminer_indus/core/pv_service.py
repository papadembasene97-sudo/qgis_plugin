...
class PVService:
    ...
    def _date_rank(self, feat) -> tuple:
        ...
        """
        candidates = [
            "date_visite", "date_contre_visite", "date_contrevisite", "date_contre_visite",
            "date_observation", "date_obs", "date_pv", "date_ctrl", "date_controle",
    ...
    def _establishment_key(self, feat) -> str:
        ...
        # IMPORTANT:
        # - on évite volontairement num_pv/id (identifiants de visite, pas d'établissement)
        # - on privilégie des identifiants "site/établissement/ouvrage"
        fields = [
            "id_etablissement", "id_etab", "id_site", "id_batiment",
            "id_maison", "id_ouvrage", "idouvrage"
        ]
        for name in fields:
            if feat.fields().indexOf(name) >= 0:
                ...
                if v:
                    return f"{name}:{v}"
        # fallback adresse + commune (souvent stable pour un même établissement)
        addr = ""
        comm = ""
        for nm in ("adresse", "addr", "rue"):
            if feat.fields().indexOf(nm) >= 0:
                addr = str(feat.attribute(nm) or "").strip()
                if addr:
                    break
        for nm in ("commune", "ville"):
            if feat.fields().indexOf(nm) >= 0:
                comm = str(feat.attribute(nm) or "").strip()
                if comm:
                    break
        if addr:
            return f"addr:{addr.lower()}|comm:{comm.lower()}"
        g = feat.geometry()
        if g:
            try:
                p = g.asPoint()
                # Superpositions visite/contre-visite : arrondi volontaire un peu plus large.
                return f"geom:{round(p.x(), 1)}:{round(p.y(), 1)}"
            except Exception:
                pass
